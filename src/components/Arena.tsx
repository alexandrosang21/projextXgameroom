...
export default function Arena() {
  ...
  const [gameStatus, setGameStatus] = useState<"WAITING" | "FIGHTING" | "GAME_OVER">("WAITING");
  ...
  const handleRemoteAction = (data: { id: string; type: string }) => {
    ...
    setPlayers((prev) => ({
      ...prev,
      [data.id]: { ...prev[data.id], action: data.type === "BLOCK_START" ? "BLOCK" : data.type },
    }));
    ...
    if (data.type === "ATTACK" || data.type === "SPECIAL") {
      ...
      if (myId && myId !== data.id) {
        ...
        if (me) {
          ...
          // Sync my new state
          socketRef.current?.emit("update-state", { health: newHealth });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        case "a": // Attack
          ...
          setPlayers((prev) => ({
             ...prev, 
             [myId]: { ...prev[myId], action: "ATTACK", energy: Math.min(100, prev[myId].energy + 5) } 
          }));
          socketRef.current?.emit("update-state", { energy: Math.min(100, players[myId].energy + 5) });
          setTimeout(() => setPlayers(prev => ({ ...prev, [myId]: { ...prev[myId], action: null } })), 200);
          break;
        
        case "s": // Block
          socketRef.current?.emit("action", { type: "BLOCK_START" });
          socketRef.current?.emit("update-state", { isBlocking: true });
          setPlayers((prev) => ({ ...prev, [myId]: { ...prev[myId], action: "BLOCK", isBlocking: true } }));
          break;
        ...
        case "d": // Special
          if (players[myId].energy >= 50) {
            ...
            setPlayers((prev) => ({ 
              ...prev, 
              [myId]: { ...prev[myId], action: "SPECIAL", energy: prev[myId].energy - 50 } 
            }));
            socketRef.current?.emit("update-state", { energy: players[myId].energy - 50 });
            setTimeout(() => setPlayers(prev => ({ ...prev, [myId]: { ...prev[myId], action: null } })), 500);
          }
          break;
      }
    };
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "s") {
        ...
        setPlayers((prev) => ({ ...prev, [myId]: { ...prev[myId], action: null, isBlocking: false } }));
      }
    };
    ...
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 text-center">
          <h2 className="text-2xl font-bold mb-6 text-white">Enter the Arena</h2>
          <input
            type="text"
    ...
    <div className="w-full h-full flex flex-col items-center justify-center relative">
      ...
      {gameStatus === "WAITING" && (
        <div className="absolute inset-0 bg-black/80 z-50 flex items-center justify-center">
          <div className="text-center animate-pulse">
            ...
            <p className="text-slate-400">Share the URL to invite a fighter</p>
          </div>
        </div>
      )}
      ...
      {gameStatus === "GAME_OVER" && (
        <div className="absolute inset-0 bg-black/90 z-50 flex items-center justify-center">
          <div className="text-center">
            <h2 className="text-6xl font-black text-yellow-500 mb-4">K.O.</h2>
      ...
      <div className="absolute bottom-8 flex gap-8 text-slate-500 text-sm font-mono">
        <div className="flex flex-col items-center">
          <div className="w-8 h-8 border border-slate-600 rounded flex items-center justify-center mb-1">A</div>
          <span>ATTACK</span>
        </div>
        <div className="flex flex-col items-center">
          <div className="w-8 h-8 border border-slate-600 rounded flex items-center justify-center mb-1">S</div>
          <span>BLOCK</span>
        </div>
        <div className="flex flex-col items-center">
          <div className="w-8 h-8 border border-slate-600 rounded flex items-center justify-center mb-1">D</div>
          <span>SPECIAL</span>
        </div>
      </div>
    </div>
